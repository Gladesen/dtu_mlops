...
a suboptimal workflow. Therefore, for truly getting "work done" you will need a good editor / IDE.
...
* The side bar: The side bar has different functionality depending on what extension you have open.
    In most cases, the side bar will just contain the file explorer.

* The editor: This where your code is. VS code supports a number of layouts in the editor (one column, two columns etc.).
...
The instructions below are specific to Visual Studio code but we recommend that you still try to answer the questions
if you are using another editor. In the `exercise_files` folder belonging to this session we have put cheat sheets for VS code
...
1. VS code is a general editor for many languages and to get proper *python* support we need to install some
    extensions. In the `action bar` go to the `extensions` tab and search for `python` in the marketplace. We highly recommend installing the following packages:
...
3. One of the most useful tools in VS Code is the ability to navigate a whole project using the built-in
    `Explorer`. To really take advantage of the VS code you need to make sure what you are working on a project.
...
However, they often lead to pain points when the models actually need to be deployed. We highly recommend reading section
...
All this said there exists at least one simple tool to make notebooks work better in a production setting. It's called
